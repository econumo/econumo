...
func newFaultService(h *harness, users appoauth.Users, ids appoauth.Identities, states appoauth.States, hands appoauth.Handoffs, allowRegistration bool) *appoauth.Service {
	return appoauth.NewService(h.providers, users, ids, states, hands, h.db.TX, h.clock, nil, "https://app.example.test", allowRegistration, false)
}
...
func TestStartLogin_DiscoveryFails(t *testing.T) {
	...
	svc2 := appoauth.NewService(unreachable, h.users, h.ids, h.states, h.hands, h.db.TX, h.clock, nil, "https://app.example.test", true, false)
