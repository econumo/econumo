...
func (s *Service) buildElementsLimits(ctx context.Context, b *budgetAggregate, f filters) (map[string]budgetedAmount, error) {
	...
	elemByID := map[string]*dombudget.BudgetElement{}
	...
	for _, sl := range summed {
		key := elementKey(sl.ExternalID, dombudget.ElementType(sl.Type))
		amt := data[key]
		if amt.budgeted.String() == "" {
			amt.budgeted = vo.NewDecimal("0")
		}
	...
	count := func(periodStart, periodEnd time.Time, current bool) error {
		...
		for _, row := range rows {
			...
			if row.TagID != nil && *row.TagID != "" {
				key = elementKey(*row.TagID, dombudget.ElementTag)
			} else {
				key = elementKey(row.CategoryID, dombudget.ElementCategory)
			}
			es := data[key]
			if es == nil {
...
func elementKey(id string, t dombudget.ElementType) string {
	return fmt.Sprintf("%s-%s", id, t.Alias())
}

// convItem builds a ConvertItem from an amountSpent into a target currency.
func convItem(a amountSpent, to vo.Id) domcurrency.ConvertItem {
	return domcurrency.ConvertItem{
