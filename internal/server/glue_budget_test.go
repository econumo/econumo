...
func TestBudgetAccountLookup_AccountsByIDs_IncludesDeletedInInputOrder(t *testing.T) {
	...
	f.Account(fixture.Account{ID: live.String(), UserID: u, Name: "Rainy day", Icon: "savings", Type: int(model.TypeSavings)})
	...
	if got[1].Name != "Rainy day" || got[1].Icon != "savings" || got[1].Type != model.TypeSavings {
		t.Fatalf("savings member view = %+v, want name/icon/type carried through", got[1])
	}
	if got[0].Type != model.TypeCreditCard || got[0].Name != "Account" || got[0].Icon != "wallet" {
