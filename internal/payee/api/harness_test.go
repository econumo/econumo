...
func newHarness(t *testing.T) *harness {
	...
	handlers := handlerpayee.NewHandlers(svc, readSvc)

	h := router.New(router.Deps{
		Cfg:         cfg,
		DB:          nil,
		RegisterAPI: handlerpayee.RegisterAPI(handlers, authstub.Authenticator{}),
	})
...
// allSortKeys reads every payee's stored sort key, so a test can assert how many
// rows a single move actually rewrote.
func (h *harness) allSortKeys(t *testing.T) map[string]string {
	t.Helper()
	rows, err := h.db.Query(`SELECT id, sort_key FROM payees`)
	if err != nil {
