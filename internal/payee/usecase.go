...
// from a column.
func assignPositions(items []model.PayeeResult) {
	for i := range items {
		items[i].Position = i
	}
}
...
func (s *Service) itemResult(ctx context.Context, userID vo.Id, p *model.Payee) (model.PayeeResult, error) {
	res := toResult(p)
	items, err := s.listResults(ctx, userID)
	if err != nil {
		return model.PayeeResult{}, err
	}
	id := p.ID.String()
