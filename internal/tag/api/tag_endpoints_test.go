...
func TestSortTagList_ByName(t *testing.T) {
	h := newHarness(t)
	token := h.issueToken(t)

	createTag(t, h, token, tagID1, "#zebra")
	createTag(t, h, token, tagID2, "#apple")
	createTag(t, h, token, tagID3, "#mango")

	status, env := h.do(t, http.MethodPost, "/api/v1/tag/sort-tag-list", token,
...
func TestSortTagList_ByUsage(t *testing.T) {
	h := newHarness(t)
	token := h.issueToken(t)

	createTag(t, h, token, tagID1, "#alpha")
	createTag(t, h, token, tagID2, "#beta")
	// #gamma is the LAST alphabetical tag; seeding it as the most-used proves
	// usage-desc sorts by count (not by name).
	idGamma := createTag(t, h, token, tagID3, "#gamma")
	...
	for _, offset := range []int{-3, -2, -1} {
		f.Transaction(fixture.Transaction{
			UserID: seedUserID, AccountID: acctID, TagID: idGamma,
	...
	if len(names) == 0 || names[0] != "#gamma" {
		t.Fatalf("usage-desc order = %v, want the most-used tag (#gamma) first", names)
	}
}

func TestSortTagList_Validation(t *testing.T) {
	h := newHarness(t)
	token := h.issueToken(t)
	cases := []struct {
		body    map[string]any
	...
	for _, tc := range cases {
		status, env := h.do(t, http.MethodPost, "/api/v1/tag/sort-tag-list", token, tc.body)
		if status != http.StatusBadRequest {
