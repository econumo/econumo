...
// The recurring service depends on the account service (for owner/grant
// checks) and the transaction service (to post instances later), so both are
// wired here exactly as the transaction harness builds them.
