...
func newHarness(t *testing.T) *harness {
	...
	repo := labelrepo.NewSQLiteRepo(txm)
	readRepo := labelrepo.NewReadRepo("sqlite", txm)
	...
	svc := applabel.NewService(repo, txm, opGuard, clk, readRepo, accountAccess)
	readSvc := applabel.NewReadService(readRepo)
	handlers := handlerlabel.NewHandlers(svc, readSvc)

	h := router.New(router.Deps{
		Cfg:         cfg,
		DB:          nil,
		RegisterAPI: handlerlabel.RegisterAPI(handlers, authstub.Authenticator{}),
	})
...
// seedLabel inserts a label row directly (bypassing the API) for the given
// owner, for tests that need a pre-existing label.
func (h *harness) seedLabel(t *testing.T, id, ownerID, name string, position int, archived bool) {
	t.Helper()
	fixture.New(t, h.tdb).Label(fixture.Label{
		ID:       id,
		UserID:   ownerID,
		Name:     name,
		Position: position,
...
// labelItem is the wire shape of a label result. Tests assert key presence and
// types, including isArchived as a number, the "2006-01-02 15:04:05" timestamp
// format, and the icon field label carries (unlike tag).
