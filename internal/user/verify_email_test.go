...
func newVerifySvcClock(t *testing.T, db *dbtest.DB, cap *captureMailer, enabled bool) (*appuser.Service, *testClock) {
	...
	return appuser.NewService(repo, db.TX, enc, hasher, tokens, server.NewUserCurrencyLookup(lookup), budgets,
		...
		ecRepo, nil,
		appuser.FixedAvatarPicker(appuser.DefaultAvatar), clk, nil, true, 0, enabled, true), clk
}
...
func newVerifySvcLimited(t *testing.T, db *dbtest.DB, cap *captureMailer, limit int) (*appuser.Service, *testClock) {
	...
	return appuser.NewService(repo, db.TX, enc, hasher, tokens, server.NewUserCurrencyLookup(lookup), budgets,
		...
		evRepo, mailer.NewVerifySender(cap, "noreply@econumo.test", ""),
		ecRepo, nil,
		appuser.FixedAvatarPicker(appuser.DefaultAvatar), clk, limiter, true, 0, true, true), clk
}
