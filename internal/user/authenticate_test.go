...
func newAuthEnvFull(t *testing.T) (*appuser.Service, *userrepo.AccessTokenRepo, *testClock, vo.Id, *userrepo.PasswordRequestRepo) {
	...
	svc := appuser.NewService(repo, db.TX, enc, hasher, tokens, server.NewUserCurrencyLookup(lookup), budgets, pwreqs, nil, appuser.FixedAvatarPicker(appuser.DefaultAvatar), clk, nil, false)
...
func TestAuthenticate_HappyPathAndCurrentIds(t *testing.T) {
	...
	gotUser, gotTok, err := svc.Authenticate(context.Background(), "eco_ses_happy")
	if err != nil {
...
func TestAuthenticate_UnknownToken401(t *testing.T) {
	...
	_, _, err := svc.Authenticate(context.Background(), "eco_ses_bogus")
...
func TestAuthenticate_ExpiredSession401(t *testing.T) {
	...
	if _, _, err := svc.Authenticate(context.Background(), "eco_ses_expired"); !isUnauthorized(err) {
		t.Fatalf("want Unauthorized, got %v", err)
	}
}
...
func TestAuthenticate_RevokedToken401(t *testing.T) {
	...
	if _, _, err := svc.Authenticate(ctx, "eco_ses_revoked"); !isUnauthorized(err) {
		t.Fatalf("want Unauthorized, got %v", err)
	}
}
...
func TestAuthenticate_SlidingTouchThrottled(t *testing.T) {
	...
	if _, _, err := svc.Authenticate(ctx, "eco_ses_touch"); err != nil {
		t.Fatalf("Authenticate: %v", err)
	}
...
func TestAuthenticate_PATExpiryNeverSlides(t *testing.T) {
	...
	clk.now = authT0.Add(6 * time.Minute)
	if _, _, err := svc.Authenticate(ctx, "eco_pat_fixed"); err != nil {
		t.Fatalf("Authenticate: %v", err)
	}
	row, _ := tokens.GetByID(ctx, tokID)
	if !row.LastUsedAt.Equal(clk.now) {
