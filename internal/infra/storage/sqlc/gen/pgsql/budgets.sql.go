...
func (q *Queries) GetBudgetLimit(ctx context.Context, arg GetBudgetLimitParams) (GetBudgetLimitRow, error) {
	row := q.db.QueryRowContext(ctx, getBudgetLimit, arg.ElementID, arg.Period)
	var i GetBudgetLimitRow
	err := row.Scan(
		&i.ID,
		&i.ElementID,
		&i.Period,
...
type ListBudgetLimitsForPeriodRow struct {
	ID        string
	ElementID string
	Period    time.Time
	CreatedAt time.Time
...
func (q *Queries) ListBudgetLimitsForPeriod(ctx context.Context, arg ListBudgetLimitsForPeriodParams) ([]ListBudgetLimitsForPeriodRow, error) {
	rows, err := q.db.QueryContext(ctx, listBudgetLimitsForPeriod, arg.BudgetID, arg.Period)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListBudgetLimitsForPeriodRow{}
	for rows.Next() {
		var i ListBudgetLimitsForPeriodRow
		if err := rows.Scan(
			&i.ID,
			&i.ElementID,
			&i.Period,
