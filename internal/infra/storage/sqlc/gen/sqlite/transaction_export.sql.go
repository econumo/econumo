...
type ListExportAccountsForUserParams struct {
	UserID   string
	UserID_2 string
}
...
func (q *Queries) ListExportAccountsForUser(ctx context.Context, arg ListExportAccountsForUserParams) ([]ListExportAccountsForUserRow, error) {
	rows, err := q.db.QueryContext(ctx, listExportAccountsForUser, arg.UserID, arg.UserID_2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
