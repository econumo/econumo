...
// owner themselves.
func (s *IdentitySender) SendIdentityLinked(ctx context.Context, to, name, provider, lang string) error {
	return s.send(ctx, to, "emails.identity_linked", map[string]any{"name": name, "provider": provider}, lang)
}
...
// lockout. lang follows the same rule as SendIdentityLinked.
func (s *IdentitySender) SendIdentityUnlinked(ctx context.Context, to, name, provider, lang string) error {
	return s.send(ctx, to, "emails.identity_unlinked", map[string]any{"name": name, "provider": provider}, lang)
}

func (s *IdentitySender) send(ctx context.Context, to, key string, params map[string]any, lang string) error {
	if lang == "" {
