...
func (r *Repo) SaveFolder(ctx context.Context, f *model.BudgetFolder) error {
	return r.q.UpsertBudgetFolder(ctx, r.db(ctx), upFolderP{
		...
		Position: f.Position, SortKey: string(f.SortKey), CreatedAt: f.CreatedAt, UpdatedAt: f.UpdatedAt,
	})
}
...
func (r *Repo) SaveElement(ctx context.Context, e *model.BudgetElement) error {
	return r.q.UpsertBudgetElement(ctx, r.db(ctx), upElementP{
		...
		CreatedAt: e.CreatedAt, UpdatedAt: e.UpdatedAt, Position: e.Position, SortKey: string(e.SortKey),
	})
}
...
func hydrateFolder(row folderRow) (*model.BudgetFolder, error) {
	...
	return &model.BudgetFolder{ID: id, BudgetID: budgetID, Name: row.Name, Position: row.Position, SortKey: sortkey.Key(row.SortKey), CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}, nil
}
...
func hydrateElement(row elementRow) (*model.BudgetElement, error) {
	...
	return &model.BudgetElement{ID: id, BudgetID: budgetID, ExternalID: externalID, Type: model.ElementType(row.Type), CurrencyID: currencyID, FolderID: folderID, Position: row.Position, SortKey: sortkey.Key(row.SortKey), CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}, nil
}
