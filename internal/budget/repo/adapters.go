...
// expressed purely in model.CategoryMeta/TagMeta/PayeeMeta, so this file
...
func (l *MetadataLookup) PayeesByOwners(ctx context.Context, userIDs []vo.Id) ([]model.PayeeMeta, error) {
	return l.payees.PayeesByOwners(ctx, userIDs)
}

// CategoriesByOwners returns all categories owned by the given users.
func (l *MetadataLookup) CategoriesByOwners(ctx context.Context, userIDs []vo.Id) ([]model.CategoryMeta, error) {
	return l.categories.CategoriesByOwners(ctx, userIDs)
}

// TagsByOwners returns all tags owned by the given users.
func (l *MetadataLookup) TagsByOwners(ctx context.Context, userIDs []vo.Id) ([]model.TagMeta, error) {
