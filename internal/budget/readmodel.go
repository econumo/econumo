// This file holds the read model: the row types the budget read repo returns
// and the ReadModel port the BudgetBuilder depends on — the budget reports
// (account balances/report/holdings + per-element spending) and the limit
// aggregations. All money values are raw NUMERIC(19,8) strings; the builder
// normalizes via vo.DecimalNumber.
package budget
