// Package budget is the budget aggregate's domain layer: the Budget aggregate
// root and its related entities (BudgetAccess, BudgetFolder, BudgetEnvelope,
// BudgetElement, BudgetElementLimit) plus the value objects (ElementType,
// UserRole). Names (budget/folder/envelope) reuse the generic 3-64 rule, so they
// are plain validated strings via the shared name helper rather than dedicated
// types.
package budget
