...
func (b *Builder) ImportSource(s ImportSource) string {
	...
	b.insert(`INSERT INTO import_sources (id, user_id, provider, name, credential_ciphertext, status, last_synced_at, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)`,
		id, s.UserID, provider, s.Name, nullable(s.CredentialCiphertext), status, now, now)
	return id
}
...
func (b *Builder) ImportRule(r ImportRule) string {
	...
	b.insert(`INSERT INTO import_rules (id, user_id, source_id, action, match_field, match_type, match_value, is_case_sensitive, target_category_id, target_payee_id, target_tag_id, priority, created_at, updated_at)
		...
		nullable(r.CategoryID), nullable(r.PayeeID), nullable(r.TagID), r.Priority, now, now)
