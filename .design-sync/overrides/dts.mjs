// forked from design-sync lib/dts.mjs — shadcn-style flat exports (CardHeader,
// DialogTrigger, …) carry no namespace/static compound signal; this fork adds
// same-source-file prefix grouping in loadDts so subcomponents fold under
// their parents instead of emitting ~340 standalone cards.
...
export function loadDts(typesRoot) {
  ...
  // fork: component names bucketed by declaring .d.ts file, for prefix grouping.
  ...
  if (sf) for (const [name, decls] of sf.getExportedDeclarations()) {
    ...
    // fork: record the declaring file for same-file prefix grouping below.
    {
      const fp = d.getSourceFile().getFilePath();
      (byFile.get(fp) ?? byFile.set(fp, new Set()).get(fp)).add(name);
    }
  ...
  // fork: flat shadcn-style exports — when a component shares its declaring
  // file with a PascalCase-prefix sibling (card.d.ts: Card + CardHeader),
  // register the longest same-file prefix as its compound parent so
  // partitionSubcomponents folds it. Cross-file prefixes (Alert vs
  // AlertDialog) never match, so standalone lookalikes stay roots.
  for (const fileNames of byFile.values()) {
    for (const n of fileNames) {
      const parts = n.match(/[A-Z][a-z0-9]*/g) ?? [];
      for (let i = parts.length - 1; i >= 1; i--) {
        const prefix = parts.slice(0, i).join('');
        if (prefix === n || !fileNames.has(prefix)) continue;
        const suffix = parts.slice(i).join('');
        const arr = compounds.get(prefix) ?? [];
        if (!arr.includes(suffix)) { arr.push(suffix); compounds.set(prefix, arr); }
        break;
      }
    }
  }
