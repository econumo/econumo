...
A feature-DEDICATED infra package (serving only X) folds into `internal/X/repo` (Task 3: openexchangerates — spec amended to match); a cross-feature adapter extracts to server glue; a redundant compile-time assertion gets deleted per the rule below. Case (c), from Task 4: a MULTI-feature adapter only partly touching X gets method-level surgery — extract only the X-touching methods to a feature-prefixed glue type in server; the residual delegates through a small port interface declared in the residual's own package and typed ONLY in the residual's own/app types (so the leaf never imports X); wire the glue implementation in server.go. Every extracted method byte-equivalent; the runtime call graph must be provably identical (reference: Task 4's `importAccountPort`).
