...
- `internal/<feature>/mcp/` — per-feature tool/resource/prompt registration
  ...
  `econumo://connections` resource / `list_connections` tool, owned by
  `connection/mcp` — no cross-feature port is needed for it; `user/mcp` stays
  profile-only.
...
| `econumo://user` | current user profile: id, name, email, avatar, base currency |
...
6. `list_accounts`, `list_categories`, `list_tags`, `list_payees`,
   ...
   input; each returns exactly the data its sibling `econumo://` resource
   serves, via the same service call. Added because resources are
   *application-controlled*: in Claude Desktop (and other clients that don't
   auto-attach resources) the model cannot read them on its own — the user
   has to attach each resource manually — so a model-driven client has no way
   to discover account, category, payee, tag, currency, budget, connection or
   user ids without these tools. The `list_*` naming (rather than e.g.
   `get_accounts`) anticipates future per-object write tools (`list_accounts`
   → `create_account` → ...), matching the existing
   `list_transactions`/`create_transaction`/... family. Resources stay for
   clients that do attach them (cheaper: no tool-call round trip, and they
   don't clutter the model's tool list).
...
struct type for schema inference even when a tool takes no arguments) and
reuse the same result DTOs / doc structs their sibling resource returns, so
output schemas are the frozen REST shapes (or, for `list_currencies`, the
same ad-hoc wrapper struct the resource already uses).
