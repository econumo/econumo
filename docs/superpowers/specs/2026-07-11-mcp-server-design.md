...
6. `list_accounts`, `list_categories`, `list_tags`, `list_payees`,
   `list_currencies`, `list_budgets`, `get_user` — no input; each returns
   exactly the data its sibling `econumo://` resource serves, via the same
   service call. Added because resources are *application-controlled*: in
   Claude Desktop (and other clients that don't auto-attach resources) the
   model cannot read them on its own — the user has to attach each resource
   manually — so a model-driven client has no way to discover account,
   category, payee, tag, currency, budget or user ids without these tools.
   The `list_*` naming (rather than e.g. `get_accounts`) anticipates future
   per-object write tools (`list_accounts` → `create_account` → ...), matching
   the existing `list_transactions`/`create_transaction`/... family. Resources
   stay for clients that do attach them (cheaper: no tool-call round trip,
   and they don't clutter the model's tool list).

Tool input/output schemas are generated by the SDK from typed Go structs; the
seven `list_*`/`get_user` tools use an empty input struct (the SDK requires a
...
output schemas are the frozen REST shapes (or, for `list_currencies` and
`get_user`, the same ad-hoc wrapper struct the resource already uses).
