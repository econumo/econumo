...
export function ViewRecurringDialog({ recurring, onClose, onPost, onSkip, onEdit, onDelete, canChange, dismissible = true }: ViewRecurringDialogProps) {
  ...
  const typeLabel = t(`pages.account.preview_transaction_modal.type.${recurring.type}`)
  ...
  const scheduleLabel = t(`modals.recurring.schedule.${recurring.schedule}`)
  ...
  const accountRow = (a: typeof account) => (
    <span className="flex items-center gap-2 text-sm">
      ...
      <span className="flex-1 truncate">{a?.name ?? t('elements.account.name_hidden')}</span>
    </span>
  )

  const cards: { label: string; content: React.ReactNode }[] = []
  if (isTransfer) {
    cards.push({ label: t('pages.account.preview_transaction_modal.sender.label'), content: accountRow(account) })
    cards.push({ label: t('pages.account.preview_transaction_modal.recipient.label'), content: accountRow(accountRecipient) })
  } else {
    const label =
    ...
    if (category) {
      cards.push({
        label: t('pages.account.preview_transaction_modal.category.label'),
        content: (
          <span className="flex items-center gap-2 text-sm">
  ...
  cards.push({ label: t('modals.recurring.preview.schedule'), content: <span className="text-sm">{scheduleLabel}</span> })
  cards.push({ label: t('modals.recurring.preview.next_payment'), content: <span className="text-sm">{dayKey(recurring.nextPaymentAt)}</span> })

  return (
    <ResponsiveDialog
      open
      onOpenChange={(o) => !o && onClose()}
      title={t('modals.recurring.preview.header')}
      hideHeader
      showClose
      ...
      footer={
        <div className="flex gap-3 [&_button]:h-11">
          ...
          <Button
            ...
            className="size-11"
            disabled={!canChange}
            aria-label={t('elements.button.edit.label')}
            title={t('elements.button.edit.label')}
            onClick={onEdit}
          >
