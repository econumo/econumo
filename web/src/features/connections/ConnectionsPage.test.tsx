...
it('accept invite posts the code and renders the refreshed list', async () => {
  ...
  await user.type(await screen.findByLabelText('Invitation code'), 'aB3f9')
...
it('accept invite failure keeps the dialog open with the server message', async () => {
  ...
  renderPage()
  await user.click(await screen.findByRole('button', { name: 'Accept invitation' }))
  await user.type(await screen.findByLabelText('Invitation code'), 'zzzzz')
  await user.click(screen.getByRole('button', { name: 'Accept' }))
...
it('delete connection confirms with the name and posts the user id', async () => {
  ...
  expect(within(dialog).getByText(/Are you sure you want to remove the connection with Partner\?/)).toBeInTheDocument()
